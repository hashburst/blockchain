...
func main() {
	...
	var secret hvm_network.NodeSecret
	must(hvm_network.LoadJSON(filepath.Join(*nodeDir, "secrets.json"), &secret))
	...
	go rt.helloLoop(ctx)
	go func() {
		log.Printf("hvm-network node %d HTTP/RPC on :%d peer=%s validator=%s", self.Index, self.RPCPort, self.PeerID, self.ValidatorID)
...
func protocolConfig(m hvm_network.Manifest) blockchain.ProtocolV2Config {
