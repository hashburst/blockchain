...
BIN="$ROOT/devnet/hvm-network/bin"
MANIFEST="$OUT/manifest.json"
PIDS="$OUT/pids"
LOGS="$OUT/logs"
export MODE NODES OUT BIN

"$ROOT/devnet/hvm-network/preflight.sh"
"$ROOT/devnet/hvm-network/build.sh"
"$BIN/hvm-network-fixture" --out "$OUT" --nodes "$NODES" --mode "$MODE"
mkdir -p "$PIDS" "$LOGS"
...
cleanup_all() {
  ...
  trap - EXIT INT TERM
  if [ "$rc" -ne 0 ]; then
    bash "$ROOT/devnet/hvm-network/collect-diagnostics.sh" "$OUT" "$MODE" "$NODES" || true
  fi
  cleanup_nodes
  if [ "$MODE" = netns ]; then NODES="$NODES" "$ROOT/devnet/hvm-network/netns.sh" cleanup || true; fi
  exit "$rc"
}
trap cleanup_all EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
if [ "$MODE" = netns ]; then NODES="$NODES" "$ROOT/devnet/hvm-network/netns.sh" setup; fi
...
start_node() {
  ...
  cmd=("$BIN/hvm-network-node" --manifest "$MANIFEST" --node-dir "$OUT/node$i" --index "$i")
  if [ "$MODE" = netns ]; then
...
echo "HVM_NETWORK_CONSENSUS_START_BARRIER_OK proposer_node=$PROP_IDX bootstrap=$BOOT"
echo "HVM_NETWORK_CONSENSUS_STARTED proposer_node=$PROP_IDX bootstrap=$BOOT"
...
dump_consensus_diagnostics() {
  echo "=== HVM_NETWORK_CONSENSUS_DIAGNOSTICS ===" >&2
  for i in $(seq 0 $((NODES-1))); do
...
echo "HVM_NETWORK_FINALITY_OK finalized=$FINAL"

"$BIN/hvm-network-ctl" --manifest "$MANIFEST" --out "$OUT" --command canary
...
echo "HVM_NETWORK_MPR_FINALIZED finalized=$FINAL"
stop_consensus_all
FINAL="$(current_min_finalized)"
...
if [ "$MODE" = netns ]; then
  ...
  stop_consensus_all
  echo "HVM_NETWORK_PACKET_LOSS_OK finalized=$FINAL"
  ...
  stop_consensus_all
  echo "HVM_NETWORK_PARTITION_RECONNECT_OK isolated=$ISO finalized=$FINAL"
  ...
  stop_consensus_all
  echo "HVM_NETWORK_PROPOSER_CRASH_OK node=$CRASH finalized=$FINAL"
fi

start_consensus_alive
if [ "$NODES" -ge 5 ]; then "$BIN/hvm-network-ctl" --manifest "$MANIFEST" --out "$OUT" --command slash --target $((NODES-1)); fi
if [ "$NODES" -ge 6 ]; then "$BIN/hvm-network-ctl" --manifest "$MANIFEST" --out "$OUT" --command unbond --target $((NODES-2)); fi
stop_consensus_all

"$BIN/hvm-network-ctl" --manifest "$MANIFEST" --out "$OUT" --command status > "$OUT/final-status.txt"
echo "HVM_NETWORK_RUN_OK mode=$MODE nodes=$NODES out=$OUT"
