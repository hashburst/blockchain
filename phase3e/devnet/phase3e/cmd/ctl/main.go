...
func runCanary(m phase3e.Manifest, out string) error {
	...
	payoutSent := hashLabel("phase3e-private-payout-sent")
	payoutCancelled := hashLabel("phase3e-private-payout-cancelled")
	...
	auditSent := hvm.AuditBatchRequest{
		ReportID: hashLabel("phase3e-report-sent"), BatchID: hashLabel("phase3e-batch-sent"), BatchIndex: 0, BatchCount: 1,
		Pool: "ViaBTC", Currency: "DOGE", CurrencyDecimals: 8, SourceTimestamp: ts,
		EvidenceHash: hashLabel("phase3e-auditing-evidence-sent"), Items: []hvm.PayoutItem{baseItem(payoutSent, "1500000000")},
	}
	...
	extTx := strings.TrimPrefix(hashLabel("phase3e-public-doge-transfer"), "0x")
	transfer := hvm.ExternalTransfer{
		...
		ObservedTimestamp: ts + 1, EvidenceHash: hashLabel("phase3e-sent-api-evidence"),
	}
	...
	auditCancelled := hvm.AuditBatchRequest{
		ReportID: hashLabel("phase3e-report-cancel"), BatchID: hashLabel("phase3e-batch-cancel"), BatchIndex: 0, BatchCount: 1,
		Pool: "ViaBTC", Currency: "DOGE", CurrencyDecimals: 8, SourceTimestamp: ts + 2,
		EvidenceHash: hashLabel("phase3e-auditing-evidence-cancel"), Items: []hvm.PayoutItem{baseItem(payoutCancelled, "250000000")},
	}
	...
	cancelled := hvm.TransitionBatchRequest{
		Status: hvm.StatusCancelled, EvidenceHash: hashLabel("phase3e-cancel-evidence"), ObservedTimestamp: ts + 3,
		ReasonCode: "DEVNET_REJECTED", ReasonHash: hashLabel("phase3e-cancel-reason"),
	...
	fmt.Printf("PHASE3E_MPR_CANARY_OK contract=%s sent=%s cancelled=%s user_ref=%s wallet_ref=%s\n", contract, payoutSent, payoutCancelled, userRef, walletRef)
	return nil
}

func runSlash(m phase3e.Manifest, out string, target int) error {
	...
	var sec phase3e.NodeSecret
	if err := phase3e.LoadJSON(filepath.Join(out, fmt.Sprintf("node%d", target), "secrets.json"), &sec); err != nil {
		return err
	}
	...
	a, err := consensus.NewSignedPrevote(m.ChainID, height, 0, hashLabel("phase3e-equivocation-a"), root, m.Nodes[target].ValidatorID, consKey)
	if err != nil {
		return err
	}
	b, err := consensus.NewSignedPrevote(m.ChainID, height, 0, hashLabel("phase3e-equivocation-b"), root, m.Nodes[target].ValidatorID, consKey)
	if err != nil {
		return err
	}
	...
	fmt.Printf("PHASE3E_SLASH_OK target=%d validator=%s tx=0x%s\n", target, m.Nodes[target].ValidatorID, tx.HashHex())
	return nil
}

func runUnbond(m phase3e.Manifest, out string, target int) error {
	if target < 0 || target >= len(m.Nodes) {
		return fmt.Errorf("target outside manifest")
	}
	var sec phase3e.NodeSecret
	...
	fmt.Printf("PHASE3E_UNBOND_OK target=%d validator=%s tx=0x%s\n", target, m.Nodes[target].ValidatorID, tx.HashHex())
	return nil
}

func runStatus(m phase3e.Manifest) error {
...
func callContract(m phase3e.Manifest, signer *wallet.Wallet, contract, method string, args interface{}, limit uint64) (hvm.Receipt, error) {
...
func getPayout(m phase3e.Manifest, sender, contract, payoutID string) (hvm.PayoutCurrent, error) {
...
func submitAndWait(m phase3e.Manifest, tx *protocolv2.TransactionV2, timeout time.Duration) (hvm.Receipt, error) {
...
func querySequence(m phase3e.Manifest, address string) (uint64, error) {
	var raw string
...
func nextValidatorSet(m phase3e.Manifest) (uint64, string, error) {
	var status struct {
...
func loadRecorder(out string) (*wallet.Wallet, error) {
	var sec phase3e.RecorderSecret
	if err := phase3e.LoadJSON(filepath.Join(out, "secrets", "recorder.json"), &sec); err != nil {
		return nil, err
	}
	return wallet.FromPrivateKeyHex(sec.PrivateHex)
}

func firstRPC(m phase3e.Manifest) string {
	n := m.Nodes[0]
...
func fatalf(format string, args ...interface{}) {
	fmt.Fprintf(os.Stderr, "phase3e-ctl: "+format+"\n", args...)
	os.Exit(1)
}
