...
type validatorFixture struct {
	pub       phase3e.NodePublic
	secret    phase3e.NodeSecret
...
func main() {
	out := flag.String("out", "./devnet/phase3e/run", "output directory")
	...
	must(err)
	safeMarker := string(os.PathSeparator) + "devnet" + string(os.PathSeparator) + "phase3e" + string(os.PathSeparator)
	if !strings.Contains(absOut, safeMarker) || !strings.HasPrefix(filepath.Base(absOut), "run-") {
		fatalf("refusing unsafe --out %s: expected .../devnet/phase3e/run-*", absOut)
	}
	...
	manifest := phase3e.Manifest{
		...
		Validator: phase3e.ValidatorConfig{
		...
		ConsensusNetwork: phase3e.NetworkConfig{
	...
	must(phase3e.WriteJSON(filepath.Join(absOut, "secrets", "recorder.json"), phase3e.RecorderSecret{PrivateHex: hex.EncodeToString(recorder.PrivateKeyBytes())}, 0o600))
	...
	for i, v := range fixtures {
		...
		must(phase3e.WriteJSON(filepath.Join(nodeDir, "secrets.json"), v.secret, 0o600))
	}
	must(phase3e.WriteJSON(filepath.Join(absOut, "manifest.json"), manifest, 0o644))
	fmt.Printf("PHASE3E_FIXTURE_OK out=%s nodes=%d bootstrap_height=%d consensus_height=%d\n", absOut, *nodesN, manifest.BootstrapHeight, manifest.ConsensusActivationHeight)
}
...
func makeValidator(i int, mode string) validatorFixture {
	...
	nodeID := fmt.Sprintf("phase3e-%s-%d", class, i)
	tep := sha256.Sum256([]byte("phase3e-tep:" + nodeID))
	pub := phase3e.NodePublic{
...
func fatalf(format string, args ...interface{}) {
	fmt.Fprintf(os.Stderr, "phase3e-fixture: "+format+"\n", args...)
	os.Exit(1)
}
