...
SAFE_OUT_ROOT="$ROOT/devnet/phase3e"
...
[ -f "$ROOT/devnet/phase3e/cmd/node/main.go" ] || { echo "ERROR: Phase 3E harness missing from source tree" >&2; exit 1; }
[ -f "$ROOT/go.mod" ] || { echo "ERROR: run from extracted Phase 3E source tree" >&2; exit 1; }

echo "PHASE3E_PREFLIGHT_OK mode=$MODE nodes=$NODES go=$GV out=$OUT"
