...
- PHASE3E_RC3_FOUR_NODE_GATES_PASSED.
...
Use Go 1.25.7, bash, jq, curl, Python 3 and standard Linux utilities. Run from phase3e/:

```sh
export GOTOOLCHAIN=local
go test ./... -count=1
go test -race ./consensus ./blockchain ./devnet/phase3e/cmd/node -count=1
go test -race ./blockchain -run '^TestDelivery' -count=20
MODE=loopback NODES=4 bash devnet/phase3e/run-all.sh
# On an isolated Linux test host with root and iproute2/tc, after loopback exits:
MODE=netns NODES=4 bash devnet/phase3e/run-all.sh
```
